...
export default function OnboardingPage() {
  ...
  // Step 2: Job preferences (skipped by share_network)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // share_network skips job prefs; others need them
        if (userIntent !== 'share_network' && !hasPrefs) { setStep(2); return; }
  ...
  // Step 1 -> 2 (job prefs for find_referrals/explore) or 3 (bonus pitch for share_network)
  const handleIntent = async () => {
    ...
    try {
      ...
      await refreshUser();
      // share_network skips job prefs, goes straight to bonus pitch
      // find_referrals and explore need job prefs first
      setStep(intent === 'share_network' ? 3 : 2);
    } catch (err) {
      setError(err.message);
    } finally {
      setSaving(false);
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="mb-6 flex items-center gap-1" role="progressbar" aria-valuenow={step} aria-valuemin={1} aria-valuemax={TOTAL_STEPS} aria-label={`Onboarding step ${step} of ${TOTAL_STEPS}`}>
          {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map((s) => {
            ...
            if (s === 2 && intent === 'share_network') return null;
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { value: 'share_network', title: 'I want to share my network and earn', desc: `Share your network anonymously. Your employer pays ${SOURCES.REFERRAL_BONUS_RANGE.claim} per referral hire \u2014 we send you pre-qualified candidates so you can capture those bonuses.` },
