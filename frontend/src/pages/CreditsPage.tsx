...
function UsageBar({ label, count, limit }) {
  ...
  return (
    <div>
      ...
      {!isUnlimited && (
        <div className="h-2 overflow-hidden rounded-full bg-muted" role="progressbar" aria-valuenow={count} aria-valuemin={0} aria-valuemax={typeof limit === 'number' ? limit : 100} aria-label={`${label} usage`}>
          <div
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6" role="main">
      ...
      <div className="surface-elevated p-6">
        ...
        <p className="stat-number mt-1 text-4xl text-primary">{balance?.balance ?? 0}</p>
        <div className="mt-4 flex gap-6 text-sm">
          <div>
            <p className="text-muted-foreground">Total Earned</p>
            <p className="font-semibold font-mono text-foreground">{balance?.earned_total ?? 0}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Total Spent</p>
            <p className="font-semibold font-mono text-foreground">{balance?.spent_total ?? 0}</p>
          </div>
        </div>
      </div>
      ...
      {balance?.expiring_soon > 0 && (
        <div className="rounded-lg border border-primary/30 bg-primary/10 p-4" role="alert" aria-live="polite">
      ...
      {usageData && (
        <div className="surface-raised p-5">
          ...
          {showActivity && activityLog.length > 0 && (
            <div className="mt-3 max-h-60 divide-y divide-border overflow-y-auto rounded-lg border border-border">
              {activityLog.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between px-3 py-2 text-xs">
                  <span className="text-secondary-foreground">{entry.action}</span>
        ...
        <div className="surface-raised p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">$1 = 5 credits. Credits expire after 12 months.</p>
            </div>
            <button
              disabled
              aria-disabled="true"
      ...
      <div className="surface-raised p-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            <div>
              <p>Find Referrals</p>
              <p className="text-xs text-muted-foreground font-normal">Use credits to search networks</p>
            </div>
          </Link>
          <Link
            to="/contacts"
          ...
          >
            ...
            <div>
              <p>Upload Contacts</p>
              <p className="text-xs text-muted-foreground font-normal">Earn 100 credits per CSV upload</p>
            </div>
          </Link>
          <Link
            to="/invite"
          ...
          >
            ...
            <div>
              <p>Invite & Earn</p>
              <p className="text-xs text-muted-foreground font-normal">Earn credits for every referral</p>
            </div>
          </Link>
        </div>
      </div>
