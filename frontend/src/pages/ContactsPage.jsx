  ...
  return (
    <div className="mx-auto max-w-4xl" role="main">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            {nlpResults !== null && !nlpFadingOut && (
              <span className="absolute left-2.5 top-1/2 z-10 -translate-y-1/2 rounded bg-amber-500/10 px-1.5 py-0.5 text-xs font-medium text-amber-400">AI</span>
            )}
            <input
              id="contacts-search"
              data-search-input
              ...
              className={`w-full rounded-lg border border-slate-700/50 bg-slate-800 py-2 pr-10 text-sm text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 ${nlpResults !== null && !nlpFadingOut ? 'pl-11' : 'pl-3'}`}
            />
            {searchInput.trim() && (
              <button
      ...
      {nlpLoading ? (
        ...
        nlpResults.length === 0 ? (
          <div className={`rounded-xl bg-slate-900 p-12 text-center border border-slate-700/50 transition-opacity duration-200 ${nlpFadingOut ? 'opacity-0' : 'opacity-100'}`}>
                    ...
                    {group.items.map((c) => {
                      ...
                      return (
                        <div
                          ...
                          className={`surface-interactive ${selectedContact?.id === c.id ? 'ring-1 ring-amber-500/50 bg-slate-800/50' : ''}`}
                        >
        ...
        filter ? (
          <div className="rounded-xl bg-slate-900 p-12 text-center border border-slate-700/50">
            <p className="text-sm text-slate-400">No contacts match this filter.</p>
          </div>
        ) : (
          <EmptyState
            icon={
            ...
            preview={
              <div className="flex items-center justify-between rounded-lg bg-slate-800/80 px-4 py-3">
                <div className="text-left">
                  <p className="text-sm font-medium text-slate-300">Sarah Chen</p>
                  <p className="text-xs text-slate-500">Senior PM at Stripe</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-400">Strong</span>
                  <span className="text-sm font-semibold text-amber-400">82</span>
                </div>
              </div>
            }
        ...
        <div className="space-y-2">
          {contactsList.map((c, i) => {
            ...
            const borderCls = score >= 70 ? 'border-l-2 border-l-emerald-500'
              : score >= 40 ? 'border-l-2 border-l-amber-500'
              : '';
            const opacityCls = score < 20 ? 'opacity-70' : '';
            const focusedCls = (focusedIndex === i || selectedContact?.id === c.id) ? 'ring-1 ring-amber-500/50 bg-slate-800/50' : '';
            return (
              <div
                key={c.id}
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    {c.relationship_type && score >= 40 && (
                      <span className={`h-2 w-2 shrink-0 rounded-full ${score >= 70 ? 'bg-emerald-500' : 'bg-amber-500'}`} aria-hidden="true" />
                    )}
                  </div>
                  <div className="flex items-center gap-3">
                    {score >= 70 && (
