...
function setCache(data: CompletedState) {
...
export default function ReferralJourney({ variant = 'full' }: ReferralJourneyProps) {
  const { user } = useAuth();
  const [completed, setCompleted] = useState<CompletedState>(getCache);
  const [loading, setLoading] = useState(true);
  const [dismissed, setDismissed] = useState(
    () => localStorage.getItem(DISMISSED_KEY) === 'true',
  );

  const isNH = user?.intent === 'share_network';
...
function FullJourney({ stages, completed, currentIndex, allComplete, persona, onDismiss }: FullJourneyProps) {
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                type="button"
                onClick={onDismiss}
                className="rounded-md border border-border px-3 py-1.5 text-xs text-muted-foreground hover:text-secondary-foreground"
              >
                Dismiss
              </button>
            </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <Link
            ...
            className={isTreb
              ...
              : 'mt-3 inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 transition-colors'}
          >
            {current.cta}
          </Link>
        </div>
...
function CompactBreadcrumb({ stages, completed, currentIndex, persona }: CompactBreadcrumbProps) {
  const Avatar = persona === 'treb' ? TrebAvatar : KeevsAvatar;
  const isTreb = persona === 'treb';
  const current = stages[currentIndex];
  const href =
    ...
    <div className="flex items-center gap-3 rounded-lg border border-border bg-card/80 px-4 py-2.5">
      ...
      <Link
        ...
        className={isTreb
          ...
          : 'shrink-0 rounded-md bg-primary px-2.5 py-1 text-xs font-medium text-white hover:bg-primary/90'}
      >
        {current.cta}
      </Link>
    </div>
