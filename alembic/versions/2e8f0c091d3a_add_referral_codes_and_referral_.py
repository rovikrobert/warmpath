...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "credits_per_conversion", sa.Integer(), server_default="25", nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["target_company_id"], ["companies.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "idx_referral_codes_owner", "referral_codes", ["owner_id"], unique=False
    )
    op.create_index(
        "idx_referral_codes_target_email",
        "referral_codes",
        ["target_email"],
        unique=False,
    )
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["referral_code_id"], ["referral_codes.id"], ondelete="CASCADE"
        ),
