...
def downgrade() -> None:
    ...
    op.add_column(
        "users",
        sa.Column(
            "password_reset_sent_at", postgresql.TIMESTAMP(timezone=True), nullable=True
        ),
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "users", sa.Column("password_hash", sa.VARCHAR(length=255), nullable=True)
    )
    op.add_column(
        "users", sa.Column("oauth_provider", sa.VARCHAR(length=50), nullable=True)
    )
    op.add_column(
        "users", sa.Column("oauth_provider_id", sa.VARCHAR(length=255), nullable=True)
    )
