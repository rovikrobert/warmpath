...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=False,
            comment="Normalized lowercase key",
        ),
        sa.Column(
            "display_name",
        ...
        sa.Column(
            ...
            sa.String(length=255),
            nullable=True,
            comment="ATS board slug (null for career_page source)",
        ),
        sa.Column(
            "career_page_url",
            sa.String(length=1000),
            nullable=True,
            comment="Fallback career page URL",
        ),
        sa.Column(
            "region",
            sa.String(length=100),
            nullable=True,
            comment="Geographic region grouping",
        ),
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Last successful ATS probe timestamp",
        ),
    ...
    op.create_index(
        "idx_company_boards_key", "company_boards", ["company_key"], unique=True
    )
    op.create_index(
        "idx_company_boards_region", "company_boards", ["region"], unique=False
    )
