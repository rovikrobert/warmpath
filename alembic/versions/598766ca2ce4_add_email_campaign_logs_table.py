...
def upgrade() -> None:
    ...
    sa.UniqueConstraint('user_id', 'email_type', 'sent_date', name='uq_campaign_user_type_date')
    )
    op.create_index('idx_campaign_type_date', 'email_campaign_logs', ['email_type', 'sent_date'], unique=False)
    op.create_index('idx_campaign_user', 'email_campaign_logs', ['user_id'], unique=False)
