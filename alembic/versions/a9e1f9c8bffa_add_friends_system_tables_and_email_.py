...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('discoverable_by_contacts', sa.Boolean(), server_default='true', nullable=False))
    op.add_column('users', sa.Column('friend_list_visible', sa.Boolean(), server_default='false', nullable=False))

    # -- external_id on email_campaign_logs --
    op.add_column('email_campaign_logs', sa.Column('external_id', sa.String(length=100), nullable=True))
