...
def _add_timestamp(table: str, column: str) -> None:
    ...
    op.add_column(table, sa.Column(column, sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True))


def upgrade() -> None:
    # --- New indexes ---
    op.create_index('idx_campaign_external_id', 'email_campaign_logs', ['external_id'], unique=False)
    op.create_index('idx_pricing_benchmarks_pricing_model', 'pricing_benchmarks', ['pricing_model'], unique=False)
    op.create_index('idx_company_boards_is_active', 'company_boards', ['is_active'], unique=False)

    # --- Add updated_at to 10 tables ---
    for table in [
        'contact_companies', 'credit_transactions', 'email_campaign_logs',
        'usage_logs', 'user_blocks', 'suppression_list', 'consent_records',
        'data_requests', 'archived_credit_transactions', 'referral_conversions',
    ]:
...
def downgrade() -> None:
    ...
    for table in [
        'referral_conversions', 'archived_credit_transactions', 'data_requests',
        'consent_records', 'suppression_list', 'user_blocks', 'usage_logs',
        'email_campaign_logs', 'credit_transactions', 'contact_companies',
    ]:
        op.drop_column(table, 'updated_at')

    # --- Drop indexes ---
    op.drop_index('idx_company_boards_is_active', table_name='company_boards')
    op.drop_index('idx_pricing_benchmarks_pricing_model', table_name='pricing_benchmarks')
