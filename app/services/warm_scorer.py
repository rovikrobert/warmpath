...
def compute_recency_score(connected_on: date | None) -> float:
    ...
    if months < 6:
        ...
        return 15.0
_RELATIONSHIP_TYPE_BONUSES: dict[str, float] = {'manager': 20.0, 'former_colleague': 15.0, 'current_colleague': 10.0, 'alumni': 10.0, 'mentor': 10.0, 'friend': 5.0, 'industry_peer': 0.0, 'recruiter': -20.0, 'other': 0.0}

def compute_relationship_score(contact: Contact, profile: ConnectorProfile | None) -> tuple[float, dict]:
    ...
    if profile is None:
        ...
        return (max(0.0, min(bonus, 100.0)), {'reason': 'no connector profile set', 'relationship_type': rel_type, 'relationship_bonus': bonus})
    score = 0.0
    factors: dict = {}
    ...
    if profile.current_company and contact.current_company and _fuzzy_eq(profile.current_company, contact.current_company):
        score += 35
    ...
    if profile.industry and contact.current_company:
        ...
        factors['same_industry'] = False
    if profile.location and contact.location and _fuzzy_eq(profile.location, contact.location):
        score += 15
...
_DEPT_KEYWORDS: dict[str, list[str]] = {'engineering': ['engineer', 'developer', 'software', 'sre', 'devops', 'platform', 'backend', 'frontend', 'fullstack', 'infrastructure'], 'product': ['product manager', 'product lead', 'product owner', 'pm'], 'design': ['designer', 'ux', 'ui', 'design lead'], 'data': ['data scientist', 'data engineer', 'data analyst', 'machine learning', 'ml', 'analytics'], 'marketing': ['marketing', 'growth', 'brand', 'content', 'communications'], 'sales': ['sales', 'account executive', 'ae', 'business development', 'bdr', 'sdr'], 'finance': ['finance', 'accounting', 'controller', 'treasury', 'fp&a'], 'hr': ['hr', 'human resources', 'people', 'talent', 'recruiting', 'recruiter'], 'operations': ['operations', 'ops', 'logistics', 'supply chain'], 'legal': ['legal', 'counsel', 'compliance', 'attorney']}
_CSUITE_PATTERN = re.compile('\\b(c[eotifsm]o|chief|founder|co-founder|cofounder|president|owner|partner)\\b', re.IGNORECASE)
_VP_PATTERN = re.compile('\\b(vp|vice\\s*president|svp|evp|avp)\\b', re.IGNORECASE)
_DIRECTOR_PATTERN = re.compile('\\b(director)\\b', re.IGNORECASE)
_MANAGER_PATTERN = re.compile('\\b(manager|head\\s+of|lead|team\\s+lead|principal)\\b', re.IGNORECASE)
_SENIOR_PATTERN = re.compile('\\b(senior|sr\\.?|staff)\\b', re.IGNORECASE)
...
def compute_role_score(title: str | None, target_role: str | None=None) -> float:
    ...
    if target_role:
        ...
        if contact_dept and target_dept:
            ...
            adjacent = (contact_dept, target_dept) in [('engineering', 'product'), ('product', 'engineering'), ('engineering', 'data'), ('data', 'engineering'), ('product', 'design'), ('design', 'product'), ('marketing', 'product'), ('product', 'marketing'), ('sales', 'marketing'), ('marketing', 'sales')]
            if same_dept:
...
def compute_tenu[RESEND_KEY_REDACTED](months_at_company: int | None=None) -> float:
...
def compute_warm_score(contact: Contact, connector_profile: ConnectorProfile | None, target_role: str | None=None) -> WarmScoreResult:
    """Compute the full warm score for a single contact."""
    recency = compute_recency_score(contact.connected_on)
    relationship, relationship_factors = compute_relationship_score(contact, connector_profile)
    ...
    tenure = compute_tenu[RESEND_KEY_REDACTED](months_at_company)
    total = recency * WEIGHT_RECENCY + relationship * WEIGHT_RELATIONSHIP + role * WEIGHT_ROLE + tenure * WEIGHT_TENURE
    total = round(min(total, 100.0), 2)
    factors = {'recency': {'score': recency, 'weight': WEIGHT_RECENCY}, 'relationship': {'score': relationship, 'weight': WEIGHT_RELATIONSHIP, **relationship_factors}, 'role': {'score': role, 'weight': WEIGHT_ROLE, 'title': contact.current_title, 'target_role': target_role}, 'tenure': {'score': tenure, 'weight': WEIGHT_TENURE, 'months_at_company': months_at_company}}
    return WarmScoreResult(total_score=total, recency_score=recency, relationship_score=relationship, role_score=role, tenu[RESEND_KEY_REDACTED]=tenure, factors=factors)

def compute_referral_score(contact: Contact, user_profile: ConnectorProfile | None, target_role: str | None=None) -> ReferralScoreResult:
    ...
    if ws.total_score >= 70:
        ...
        likelihood = 'low'
    return ReferralScoreResult(total_score=ws.total_score, recency_score=ws.recency_score, relationship_score=ws.relationship_score, role_score=ws.role_score, tenu[RESEND_KEY_REDACTED]=ws.tenu[RESEND_KEY_REDACTED], referral_likelihood=likelihood, factors=ws.factors)

@timed('warm_sco[RESEND_KEY_REDACTED]')
async def batch_compute_scores(user_id: uuid.UUID, db: AsyncSession, target_role: str | None=None) -> list[WarmScore]:
    """Compute warm scores for all of a user's active contacts.

    Upserts into the warm_scores table (one row per user+contact).
    """
    result = await db.execute(select(ConnectorProfile).where(ConnectorProfile.user_id == user_id))
    profile = result.scalar_one_or_none()
    result = await db.execute(select(Contact).where(Contact.user_id == user_id, Contact.deleted_at.is_(None)))
    contacts = result.scalars().all()
    contact_ids = [c.id for c in contacts]
    existing_ws_map: dict = {}
    if contact_ids:
        ws_result = await db.execute(select(WarmScore).where(WarmScore.user_id == user_id, WarmScore.contact_id.in_(contact_ids)))
    ...
    for contact in contacts:
        ...
        if existing:
            ...
            existing.tenu[RESEND_KEY_REDACTED] = Decimal(str(ref_result.tenu[RESEND_KEY_REDACTED]))
            existing.sco[RESEND_KEY_REDACTED] = ref_result.factors
            ...
            ws = WarmScore(user_id=user_id, contact_id=contact.id, total_score=Decimal(str(ref_result.total_score)), recency_score=Decimal(str(ref_result.recency_score)), context_score=Decimal(str(ref_result.relationship_score)), role_score=Decimal(str(ref_result.role_score)), tenu[RESEND_KEY_REDACTED]=Decimal(str(ref_result.tenu[RESEND_KEY_REDACTED])), sco[RESEND_KEY_REDACTED]=ref_result.factors, referral_likelihood=ref_result.referral_likelihood, algorithm_version=ALGORITHM_VERSION, computed_at=datetime.now(timezone.utc))
            db.add(ws)
            scores.append(ws)
    await db.flush()
    return scores