...
async def get_competitor_matrix(db: AsyncSession) -> dict:
    ...
    for c in competitors:
        feats = c.features or {}
        featu[RESEND_KEY_REDACTED] = set(feats.keys()) if isinstance(feats, dict) else set()
        all_features.update(featu[RESEND_KEY_REDACTED])
    ...
    for c in competitors:
        feats = c.features or {}
        featu[RESEND_KEY_REDACTED] = set(feats.keys()) if isinstance(feats, dict) else set()
        matrix[c.name] = {f: f in featu[RESEND_KEY_REDACTED] for f in sorted_features}
