...
def classify_warm_sco[RESEND_KEY_REDACTED](score: float | None) -> str:
    """Derive warm_sco[RESEND_KEY_REDACTED] from numeric warm score.
...
async def generate_marketplace_listings(
    ...
    sco[RESEND_KEY_REDACTED]: dict[uuid.UUID, float] = {}
    if contacts:
        ...
        for ws in ws_result.scalars():
            sco[RESEND_KEY_REDACTED][ws.contact_id] = float(ws.total_score)
