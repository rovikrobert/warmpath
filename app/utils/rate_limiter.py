...
async def acqui[RESEND_KEY_REDACTED](name: str, max_concurrent: int = 5, timeout: int = 60) -> bool:
...
async def release_slot(name: str, used_redis: bool) -> None:
    ...
    Args:
        name: Limiter name.
        used_redis: True if acqui[RESEND_KEY_REDACTED] returned True (Redis path).
    """
    if not used_redis:
        # Local fallback
