...
class TestGTMPrivacyGuard:
    ...
    def test_rejects_forbidden_action_sha[RESEND_KEY_REDACTED](self):
        guard = self._get_guard()
        PrivacyViolation = self._get_violation()
        with pytest.raises(PrivacyViolation):
            guard.validate_action("sha[RESEND_KEY_REDACTED] with partners")
...
class TestStratOpsScan:
    ...
    def test_scan_readiness_sco[RESEND_KEY_REDACTED](self, _ws):
        from gtm_team.stratops.scanner import scan
