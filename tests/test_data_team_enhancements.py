...
class TestAnalystAnomalyDetection:
    ...
    def test_detect_zscore_normal(self):
        from data_team.analyst.analyst import _detect_zscore_anomaly

        values = [100, 102, 98, 101, 99, 103, 100]
        assert _detect_zscore_anomaly(values) is None

    def test_detect_zscore_spike(self):
        from data_team.analyst.analyst import _detect_zscore_anomaly

        values = [100, 102, 98, 101, 99, 103, 200]
        result = _detect_zscore_anomaly(values)
        assert result is not None
        assert result["direction"] == "high"
        assert result["z_score"] > 2.0

    def test_detect_zscore_drop(self):
        from data_team.analyst.analyst import _detect_zscore_anomaly
    ...
    def test_detect_zscore_too_few_values(self):
        from data_team.analyst.analyst import _detect_zscore_anomaly

        assert _detect_zscore_anomaly([1, 2, 3]) is None

    def test_detect_zscore_zero_std(self):
        from data_team.analyst.analyst import _detect_zscore_anomaly

        assert _detect_zscore_anomaly([5, 5, 5, 5, 5]) is None
...
class TestModelEngineerCalibration:
    ...
    def test_miscalibrated_scores_flagged(self):
        from data_team.model_engineer.model_engineer import _scan_warm_score_calibration
        ...
        mock_qe.execute_sql.return_value = [
            {"score_band": "high", "total_intros": 50, "approved": 10},
            {"score_band": "medium", "total_intros": 50, "approved": 30},
            {"score_band": "low", "total_intros": 50, "approved": 40},
        ]

        with patch("data_team.shared.query_executor.get_executor", return_value=mock_qe):
            _scan_warm_score_calibration(findings, insights, metrics)

        assert metrics["warm_score_monotonic"] is False
        assert any(f.id == "model-009" for f in findings)
        # Band rates should be inverted
        assert metrics["warm_score_band_rates"]["high"] < metrics["warm_score_band_rates"]["low"]
    ...
    def test_well_calibrated_scores(self):
        ...
        mock_qe.is_available.return_value = True
        mock_qe.execute_sql.return_value = [
            {"score_band": "high", "total_intros": 50, "approved": 40},
            {"score_band": "medium", "total_intros": 50, "approved": 25},
            {"score_band": "low", "total_intros": 50, "approved": 5},
        ]

        with patch("data_team.shared.query_executor.get_executor", return_value=mock_qe):
            _scan_warm_score_calibration(findings, insights, metrics)

        assert metrics["warm_score_monotonic"] is True
    ...
    def test_no_data_returns_early(self):
        ...
        assert metrics["warm_score_sample_size"] == 0
        assert len(findings) == 0
    ...
    def test_skips_when_unavailable(self):
        ...
        assert metrics["warm_score_calibration_available"] is False
...
class TestIntegrationScans:
    ...
    def test_model_engineer_scan_includes_calibration_metrics(self):
        ...
        assert "warm_score_calibration_available" in report.metrics
