...
class TestLocalFallback:
    ...
    async def test_local_fallback_limits_concurrency(self):
        ...
        async def try_acquire():
            nonlocal acquired
            with patch("app.utils.rate_limiter._get_redis_client", return_value=None):
                await acquire_slot("test_concurrency", max_concurrent=1, timeout=1)
                acquired += 1
        ...
        with patch("app.utils.rate_limiter._get_redis_client", return_value=None):
            await acquire_slot("test_concurrency", max_concurrent=1)

        with (
            pytest.raises((asyncio.TimeoutError, TimeoutError)),
            patch("app.utils.rate_limiter._get_redis_client", return_value=None),
        ):
            await acquire_slot("test_concurrency", max_concurrent=1, timeout=0)
...
class TestRedisLimiter:
    ...
    @pytest.mark.asyncio
    async def test_acquire_slot_uses_brpop(self):
        """acquire_slot uses BRPOP to take a token from the list."""
        mock_client = self._mock_redis()
        mock_client.llen = AsyncMock(return_value=5)
        ...
        with patch(
            ...
            used_redis = await acquire_slot("test", max_concurrent=5)

        assert used_redis is True
        mock_client.brpop.assert_called_once()

    @pytest.mark.asyncio
    async def test_acquire_slot_timeout_raises(self):
        """acquire_slot raises TimeoutError when BRPOP returns None."""
        mock_client = self._mock_redis()
        mock_client.llen = AsyncMock(return_value=5)
        ...
        with (
            ...
            await acquire_slot("test", max_concurrent=5, timeout=1)
    ...
    @pytest.mark.asyncio
    async def test_acquire_falls_back_on_redis_error(self):
        ...
        with patch(
            ...
            used_redis = await acquire_slot("test_error", max_concurrent=2)
