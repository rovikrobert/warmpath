...
@pytest.mark.asyncio
async def test_poll_railway_logs_dispatches_events_when_alert_threshold_met():
    """Railway poll creates and dispatches event when error count >= threshold."""
    from app.agent_runtime.events.railway_poll import poll_railway_logs_async
    ...
    with (
        patch("app.agent_runtime.events.railway_poll.stream_add", mock_stream_add),
        patch(
            "app.agent_runtime.events.railway_poll._fetch_recent_logs",
            return_value=log_lines,
        ),
    ):
        await poll_railway_logs_async()
...
@pytest.mark.asyncio
async def test_poll_railway_logs_skips_dispatch_below_threshold():
