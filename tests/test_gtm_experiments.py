...
async def admin_headers(client: AsyncClient) -> dict:
    ...
    async with TestSessionLocal() as s:
        ...
        await s.execute(
            update(User).where(User.email == "expadmin@test.com").values(is_admin=True)
        )
        await s.commit()
    login = await client.post(
        "/api/v1/auth/login",
        json={"email": "expadmin@test.com", "password": "Test1234!@#$"},
    )
...
async def user_headers(client: AsyncClient) -> dict:
    ...
    login = await client.post(
        "/api/v1/auth/login",
        json={"email": "expuser@test.com", "password": "Test1234!@#$"},
    )
    token = login.json()["data"]["access_token"]
    return {"Authorization": f"Bearer {token}"}
