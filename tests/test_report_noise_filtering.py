...
class TestSharedN1Scanner:
    ...
    def test_require_await_filters_sync_calls(self, tmp_path):
        """require_await=True only flags awaited calls."""
        from agents.shared.n1_scanner import scan_n_plus_one
        ...
        # Without require_await — should detect
        findings = scan_n_plus_one([f], id_prefix="T", require_await=False)
        assert len(findings) == 1

        # With require_await — should skip (no await)
        findings = scan_n_plus_one([f], id_prefix="T", require_await=True)
        assert len(findings) == 0
